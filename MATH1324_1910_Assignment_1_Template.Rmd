...
bdims_csv %>% group_by(sex) %>% summarise(Min = min(bit.di), Q1 = quantile(bit.di,probs = .25), Median = median(bit.di), Q3 = quantile(bit.di,probs = .75), Max = max(bit.di), Mean = mean(bit.di), SD = sd(bit.di), IQR = IQR(bit.di))
```
